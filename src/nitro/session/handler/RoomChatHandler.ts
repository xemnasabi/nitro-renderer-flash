...
import { FloodControlEvent, FloodTranslateControlEvent, PetRespectNoficationEvent, PetSupplementTypeEnum, PetSupplementedNotificationEvent, RemainingMuteEvent, RespectReceivedEvent, RoomUnitChatEvent, RoomUnitChatShoutEvent, RoomUnitChatWhisperEvent, RoomUnitHandItemReceivedEvent } from '../../communication';
...
export class RoomChatHandler extends BaseHandler
{
    constructor(connection: IConnection, listener: IRoomHandlerListener)
    {
        ...
        connection.addMessageEvent(new FloodTranslateControlEvent(this.onFloodTranslateControlEvent.bind(this)));
    ...
    private onFloodTranslateControlEvent(event: FloodTranslateControlEvent): void
    {
        if(!this.listener) return;
        ...
        this.listener.events.dispatchEvent(new RoomSessionChatEvent(RoomSessionChatEvent.FLOOD_TRANSLATE_EVENT, session, -1, seconds.toString(), 0, 0));
    }
