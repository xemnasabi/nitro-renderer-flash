...
export class RoomUsersHandler extends BaseHandler
{
    ...
    private onRoomUnitEvent(event: RoomUnitEvent): void
    {
        ...
        if(users && users.length)
        {
            for(const user of users)
            {
                ...
                userData.respects = user.respects;
                userData.onlinetime = user.onlinetime;
    ...
    private onRoomUnitInfoEvent(event: RoomUnitInfoEvent): void
    {
        ...
        session.userDataManager.updateRespects(parser.unitId, parser.respects);
        session.userDataManager.updateOnlineTime(parser.unitId, parser.onlinetime);
        session.userDataManager.updateDiscord(parser.unitId, parser.discord);
        session.userDataManager.updateTwitter(parser.unitId, parser.twitter);
        session.userDataManager.updateFacebook(parser.unitId, parser.facebook);
        session.userDataManager.updateInstagram(parser.unitId, parser.instagram);
        session.userDataManager.updateTikTok(parser.unitId, parser.tiktok);
        session.userDataManager.updateVerified(parser.unitId, parser.verified);
