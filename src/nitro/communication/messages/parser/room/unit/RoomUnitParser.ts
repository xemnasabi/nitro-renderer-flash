...
export class RoomUnitParser implements IMessageParser
{
    ...
    public parse(wrapper: IMessageDataWrapper): boolean
    {
        ...
        while(i < totalUsers)
        {
            ...
            const banner = wrapper.readString();
            const lang = wrapper.readString();

            const level = wrapper.readString();
            const respects = wrapper.readString();
            const onlinetime = wrapper.readString();
            const instagram = wrapper.readString();
            const discord = wrapper.readString();
            const twitter = wrapper.readString();
            const facebook = wrapper.readString();
            const tiktok = wrapper.readString();
            const verified = wrapper.readString();
